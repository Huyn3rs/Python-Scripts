    ...
    while True:
        try:
            if potential > left:
                right = next(points)
                if right < potential:
                    all_peaks.append(potential)
                left = (right if right < potential else potential)
                potential = (next(points) if right < potential else right)                    
            else:
                left = potential
                potential = next(points)
...
def compress(vit,bit): 
    for letter, boolean in zip(vit, bit):
        if boolean:
            yield letter
...
def alternate_all(*args):
    iter_list = [[letter for letter in arg] for arg in args] 
    while True:
        for iterable in iter_list:
            if iterable != []:
                yield next(iter(iterable))
                iterable.pop(0)
        if all([len(arg) == 0 for arg in iter_list]):
            break
...
class Ordered:
    ...
    def __iter__(self):
        lowest = int()
        while self.aset != set():
            number = min(self.aset)
            if number >= lowest:
                yield number
                lowest = number
            if number in self.aset:
                self.aset.remove(number)
