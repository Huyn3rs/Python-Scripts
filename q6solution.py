...
def is_min_heap(t):
    if t == None:
        return True
    elif t.right != None and t.value >= t.right.value:
        return False
    elif t.left != None and t.value >= t.left.value: 
        return False
    else:
...
class StringVar_WithHistory(StringVar):
    ...
    def set (self,value):
        ...
        if value != StringVar.get(self):
            self._h_list.append(value)
            StringVar.set(self, value)

        
    def undo (self):
        if len(self._h_list) > 1:
            self._h_list.pop()
            StringVar.set(self, self._h_list[-1])
